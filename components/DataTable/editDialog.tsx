...
export default function EditDialog({ row }: EditDialogProps) {
  ...
  const onSubmit = (values: z.infer<typeof editSchema>) => {
    ...
    toast("Job Application has been edited", {
      description:
        `Your application to ${values.company} for the ${values.position} position has been edited`,
    });
    setOpen(false);
  };
  ...
  return (
    <DialogTemplate
      ...
      desc={"Edit an existing job application"}
      open={open}
      setOpen={setOpen}
    >
      <form
